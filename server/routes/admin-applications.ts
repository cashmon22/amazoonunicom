...
async function listApplications(
  ...
  const adminSupabase = await getAdminSupabaseClient(req, res);
  ...
  const applications = (data ?? [])
    .map((row) =>
      databaseRowToApplication(row as Record<string, unknown>),
    )
    .filter(
      (application) =>
        (!search ||
...
export const mirrorApplication: RequestHandler = async (req, res) => {
  try {
    ...
    const { error } = await supabase.from("applications").insert({
      ...
      weekly_hours: stringValue(data.hours),
      previous_experience: stringValue(data.experience),
      motivation: stringValue(data.reason),
...
export const updateAdminApplicationStatus: RequestHandler = async (
  ...
  res.status(409).json({
    error:
      "Application status changes require a status column in the applications table.",
  });
};
